...
-- Insert 2 surveys per user (total 4)
INSERT INTO `surveys` (`name`, `discription`, `creator`, `s_code`) VALUES
  ('User1 Survey A','Survey A for Test User', 1, '11SURVE111'),
  ('User1 Survey B','Survey B for Test User', 1, '11SURVE112'),
  ('Admin Survey A','Survey A for Admin User', 2, '11SURVE113'),
  ('Admin Survey B','Survey B for Admin User', 2, '11SURVE114');
...
INSERT INTO `responses` (`survey_id`, `question_id`, `answer`, `responder_id`) VALUES
  (1, (SELECT id FROM questions WHERE survey_id=1 ORDER BY id LIMIT 1), 'I like the speed and simplicity', 1),
  (1, (SELECT id FROM questions WHERE survey_id=1 ORDER BY id LIMIT 1 OFFSET 1), 'A better mobile layout', 1),
  (1, (SELECT id FROM questions WHERE survey_id=1 ORDER BY id LIMIT 1 OFFSET 2), 'No further comments', 1);

-- Insert some sample responses from admin (user 2) to survey 3
INSERT INTO `responses` (`survey_id`, `question_id`, `answer`, `responder_id`) VALUES
  (3, (SELECT id FROM questions WHERE survey_id=3 ORDER BY id LIMIT 1), 'Mostly satisfied', 2),
  (3, (SELECT id FROM questions WHERE survey_id=3 ORDER BY id LIMIT 1 OFFSET 1), 'Faster ticket routing', 2),
  (3, (SELECT id FROM questions WHERE survey_id=3 ORDER BY id LIMIT 1 OFFSET 2), 'No particular story', 2);
